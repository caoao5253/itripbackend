...
public class HotelOrderController {
    ...
    public Dto<RoomStoreVO> getPreOrderInfo(@RequestBody ValidateRoomStoreVO validateRoomStoreVO, HttpServletRequest request) {
        String tokenString = request.getHeader("token");//获取token头
        ItripUser currentUser = validationToken.getCurrentUser(tokenString);//通过token字符串key去redis找用户的数据
        ItripHotel hotel = null;//酒店表对象
        ItripHotelRoom room = null;//酒店房间表对象
        try {
            ...
            if (EmptyUtils.isEmpty(validateRoomStoreVO.getHotelId())) {
                ...
                param.put("startTime", validateRoomStoreVO.getCheckInDate());//开始时间
                param.put("endTime", validateRoomStoreVO.getCheckOutDate());//结束时间
                param.put("roomId", validateRoomStoreVO.getRoomId());//房间id
                param.put("hotelId", validateRoomStoreVO.getHotelId());//酒店id

                roomStoreVO.setCheckInDate(validateRoomStoreVO.getCheckInDate());//入住时间
                roomStoreVO.setCheckOutDate(validateRoomStoreVO.getCheckOutDate());//退房时间
                roomStoreVO.setHotelName(hotel.getHotelname());
                roomStoreVO.setRoomId(room.getId());//酒店房间id
                roomStoreVO.setPrice(room.getRoomprice());//房间价格
                roomStoreVO.setHotelId(validateRoomStoreVO.getHotelId());//酒店id
    ...
    public Dto<Object> addHotelOrder(@RequestBody ItripAddHotelOrderVO itripAddHotelOrderVO, HttpServletRequest request) {
        ...
        String tokenString = request.getHeader("token");//获取请求头
        logger.debug("token name is from header : " + tokenString);
        //通过token请求头在redis获取用户相关信息
        ...
        //通过订单简短Vo获取订单信息  创建时间 结束时间 酒店id  房间id  数量
        ...
        try {
            ...
            if (flag && null != itripAddHotelOrderVO) {
                ...
                if (tokenString.startsWith("token:PC")) {
                    ...
                    itripHotelOrder.setBookingdays(2);
                }
                //支付之前生成的订单的初始状态为未支付
